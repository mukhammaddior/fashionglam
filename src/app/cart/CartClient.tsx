  ...
  return (
    <div className="flex flex-col lg:flex-row gap-12">
      <div className="w-full lg:w-2/3">
        {/* Table Header */}
        <div className="hidden md:grid md:grid-cols-12 gap-4 border-b border-gray-200 pb-4 mb-6 text-xs font-medium uppercase tracking-widest text-gray-400">
        ...
        {/* Cart Items */}
        {cart.map((item) => (
          <div key={`${item.id}-${item.size}-${item.color}`} className="flex flex-col md:grid md:grid-cols-12 gap-4 md:items-center border-b border-gray-100 py-6">
            <div className="md:col-span-6 flex gap-4 w-full">
              <Link href={`/product/${item.slug}`} className="relative w-24 h-32 md:w-20 md:h-24 bg-gray-50 shrink-0">
              ...
              <div className="flex flex-col justify-between flex-grow">
                ...
                {/* Mobile Price */}
                <div className="md:hidden mt-2 text-sm font-light">
                  ${item.price.toFixed(2)}
                </div>

                <div className="flex items-center justify-between md:block mt-4 md:mt-0">
                  <button 
                    onClick={() => removeFromCart(item.id, item.size, item.color)}
                    className="text-xs text-gray-400 hover:text-red-500 md:mt-3 flex items-center gap-1 transition-colors w-fit"
                  >
                    <Trash2 className="w-3 h-3" /> Remove
                  </button>
                  
                  {/* Mobile Quantity */}
                  <div className="md:hidden flex items-center border border-gray-200">
            ...
            <div className="hidden md:block col-span-2 text-right text-sm font-medium">
